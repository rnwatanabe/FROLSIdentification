...
%	where:
%
% 	signal1 is the input signal. It can contain multiple trials of the same system. Each trial must be in one column 
%	of the signal1 matrix.
%
% 	signal2 is the output signal. It can contain multiple trials of the same system. Each trial must be in one column 
...
%
%
% 	Dn is a vector in which each element is a string with a term found during the residue idetification. u is the input 
...
function [Dn, an, ln] = NARXNoiseModelIdentification(signal1, signal2, degree, mu, my, me, delay, dataLength, ...
   ...
   if parallel
      ...
      matlabpool close
       ...
       beta = mfrols(pn, xi_output, 1e-5, pho, s, 0);
   end
%%
   err=err(1:M0)';
   l=l(1:M0)';
